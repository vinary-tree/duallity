...
| `expand(s) -> Result<ExpansionStatus, ExpansionError>` | none | Computes `s` (if not resident) via the state source and stores it per the active [`CachePolicy`](#5-cachepolicy). **Idempotent**: re-expanding a cached state is a touch, not a recompute. The success value distinguishes `ExpandedEmpty` from `ExpandedNonempty`; invalid ids return `ExpansionError::Failure(InvalidState)` and are never cached. |
| `transitions_lazy(s) -> &[…]` | none | Ensures `s` is computed, then returns its arc slice (borrowed from `&mut self`). This compatibility method returns `&[]` if expansion fails; call `expand(s)?` first when the distinction between an empty valid state and an error matters. **Complexity: amortized $`\mathcal{O}(1)`$ cache access** — see the note below. |
...
### Complexity of `transitions_lazy` — amortized $`\mathcal{O}(1)`$
...
- **Cache access** (`touch_if_cached`) is a single `FxHashMap` probe: **amortized $`\mathcal{O}(1)`$**. Under
  `Lru`, a hit also records a fresh access tick by pushing onto a binary heap ($`\mathcal{O}(\log c)`$
  amortized for the *tick-maintenance*, with lazy heap compaction; the *lookup itself* stays $`\mathcal{O}(1)`$).
  Under `CacheAll` and `NoCache` there is no heap upkeep, so the whole access is $`\mathcal{O}(1)`$.
- **On a miss**, add $`T_{\text{compute}}(s)`$ — the state source's direct `expand_state`, which for a
  **standard Levenshtein cell is $`\mathcal{O}(1)`$** (it emits at most four transitions —
  match / substitute / insert / delete — from an inline `SmallVec<[_; 4]>` with no heap allocation) — plus
  an $`\mathcal{O}(1)`$ amortized insert (`CacheAll`) or an $`\mathcal{O}(\log c)`$ evict-then-insert (`Lru`).

So a **hot traversal that revisits states pays amortized $`\mathcal{O}(1)`$ per touch**, and a cold traversal
...
## 3. `StateSource` and `DirectStateSource` — managed and direct computation

```rust,ignore
pub trait StateSource<L, W: Semiring>: Clone + Send + Sync {
    fn compute_state(&self, request: ExpansionRequest<'_>) -> StateExpansion<L, W>;
    fn snapshot(&self) -> SourceSnapshot { SourceSnapshot::IMMUTABLE }
    fn start(&self) -> StateId;
...
A `StateSource` is the **immutable managed core** that knows how to compute *one* state on demand. Its
`ExpansionRequest` carries the state id, source snapshot, attempt number, and cooperative cancellation
token. It returns one terminal `StateExpansion`:

```rust,ignore
StateExpansion::Expanded { is_final, final_weight, transitions }
StateExpansion::Failed(ExpansionFailure)
StateExpansion::Cancelled(CancellationReason)
```

`LazyState` is no longer the source result. It is lling-llang's internal lifecycle record:
`Unexpanded → Expanding → Expanded | Failed | Cancelled`, with the winning attempt attached. Keeping
the source result separate prevents an invalid id, an empty valid state, cancellation, and a state not
yet attempted from collapsing into the same observation.

duallity additionally implements `DirectStateSource` for every source and self-sourcing automaton.
`expand_state(s)` is the allocation-conscious deterministic kernel used by duallity's specialized
caches and by tests or integrations that already own lifecycle policy. The `StateSource` implementation
first honors request cancellation, then delegates to that same kernel, so the two paths cannot drift.

Because both methods take **`&self`**, a state source can be wrapped in a
`LazyWfstWrapper<S, L, W>` and dropped straight into [`compose`](../theory/04-composition.md), with no
`&mut` plumbing. The wrapper owns attempts, snapshots, cancellation, and terminal lifecycle state;
duallity owns automaton semantics.

| Method | Preconditions | Postconditions |
|--------|---------------|----------------|
| `compute_state(request) -> StateExpansion` | request owned by lling-llang | Honors cancellation before computation, then returns `Expanded`, classified `Failed`, or `Cancelled`. The default immutable snapshot is stable; mutable sources must override it. |
| `expand_state(s) -> StateExpansion` | caller owns lifecycle concerns | Direct deterministic computation. A valid state returns `Expanded`, including valid states with zero arcs; an out-of-domain id returns `Failed(InvalidState)`. |
...
| Style | State source (the kernel) | WFST (the cache) | How direct computation stays immutable |
|-------|---------------------------|------------------|--------------------------------------------|
...
| **Self-sourcing** | the WFST *is* its own source — `GeneralizedWfst`, `WallBreakerWfst`, `RewriteWfst`, `PhoneticNfaWfst` implement `LazyWfst`, `StateSource`, and `DirectStateSource` | same object | `GeneralizedWfst` keeps its node / product / continuation registries behind `Arc<RwLock<_>>`, so it can register newly discovered states while satisfying the `&self` signature; `WallBreakerWfst` instead **pre-registers its finite result-chain state forest at construction** (WallBreaker has already materialized the accepted terms), making `expand_state` a read-only walk |
...
1. **Direct traversal** — call `expand(s)?` and then `transitions(s)` so lifecycle errors stay visible.
   Use `transitions_lazy(s)` only when treating failure as an empty transition slice is intentional.
...
|---------|---------------|----------|------------------------------|----------|
| `CacheAll` *(default)* | every computed state, as a persistent entry | none | $`\mathcal{O}(1)`$ | one-shot queries, batch jobs — the common case |
| `Lru { max_states }` | at most `max_states` persistent entries | least-recently-touched (smallest access tick) evicted before inserting a new state; deterministic | $`\mathcal{O}(1)`$ lookup + $`\mathcal{O}(\log c)`$ tick upkeep | long-lived / streaming services that must bound memory |
...
| `lookup(id) -> Option<&str>` | none | `Some(term)` if `id` was returned by a successful `intern`; `None` for an out-of-range id or the exhaustion sentinel. $`\mathcal{O}(1)`$ (a `Vec` index). |
