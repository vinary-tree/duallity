...
> to).
>
> **Defines:** the `expand → expand_state → cache` pipeline; duallity's own
...
2. On a **miss**, the free function `ensure_cached_char_state` (`src/lazy_cache.rs`) drives the fill:
   ...
   {
       if cache.touch_if_cached(state) {                    // ① already resident; report exact status
           return cached_char_state_status(cache, state);
       }
       if !is_valid(state_source, state) {                  // ② classify invalid id; never cache it
           return Err(invalid_state_error(state));
       }
       cache_char_state_expansion(                          // ③ compute once, classify, memoize success
           cache,
           state,
           state_source.expand_state(state),
       )
   }
   ```
...
3. `expand_state(s)` decodes $`s`$ into its components, resolves them against the registries
   ([architecture/05](05-registries-and-interning.md)), and returns a
   `StateExpansion::Expanded { is_final, final_weight, transitions }`, which is stored as a
   `CachedCharState`. `Failed` and `Cancelled` results become `ExpansionError`s and are not cached.
...
- **`entries`** — the resident memo, an `FxHashMap<StateId, CacheEntry<T>>`. `get`, `insert`, and
  membership are all $`\mathcal{O}(1)`$ expected.
...
keeps $`H = \mathcal{O}(N)`$. `FxHashMap` operations are $`\mathcal{O}(1)`$ expected.

| Operation | `CacheAll` | `Lru { max_states }` | `NoCache` |
|-----------|-----------|----------------------|-----------|
| `get` / `touch_if_cached` (hit) | $`\mathcal{O}(1)`$ | $`\mathcal{O}(\log H)`$ (re-touch pushes one tick) | $`\mathcal{O}(1)`$ (slot compare) |
| `insert`, first touch | $`\mathcal{O}(1)`$ amortized | $`\mathcal{O}(\log H)`$ amortized + eviction | $`\mathcal{O}(1)`$ (overwrite slot) |
| eviction of one victim | — (never) | $`\mathcal{O}(\log H)`$ amortized (lazy, skips stale) | — |
| resident memory | $`\mathcal{O}(\text{states visited})`$ | $`\mathcal{O}(N)`$ | $`\mathcal{O}(1)`$ |
| heap memory | $`0`$ (unused) | $`\mathcal{O}(N)`$ | $`0`$ (unused) |
...
1. `cache.touch_if_cached(0)` → $`0 \notin \texttt{entries}`$, `scratch` empty → **`false`** (miss).
2. `is_valid(source, 0)` → `decode(0, M) = (0, 0)`; node $`0`$ is the registered root, automaton
   $`0`$ is the live start coordinate → **`true`**.
3. `source.expand_state(0)` walks (dict root $`\times`$ automaton start), yielding
   `StateExpansion::Expanded { .. }`.
4. `CachedCharState::from_state_expansion(..)` → `Ok(cached)` and derives the exact
   `ExpandedEmpty` or `ExpandedNonempty` status.
...
1. `cache.touch_if_cached(0)` → $`0 \in \texttt{entries}`$ → `touch(0)` (a no-op under `CacheAll`) → **`true`**;
...
|-------------------|--------------|
| `expand(&mut self, s) -> Result<ExpansionStatus, ExpansionError>` | an `ensure_state` fill of the `LazyStateCache` (`ensure_cached_char_state(&mut cache, &source, s, is_valid)` in the state-source wrappers), preserving empty/non-empty, failure, and cancellation |
| `transitions_lazy(&mut self, s)` | attempt `expand`, then return `transitions(s)`; use the fallible `expand` first when errors must not collapse to `&[]` |
...
|------|--------|------------|---------|
| **StateSource** | `compute_state(&self, ExpansionRequest) -> StateExpansion` | immutable (`&self`) | managed snapshots, attempts, cancellation, and composition via `lling_llang`'s `LazyWfstWrapper` |
| **DirectStateSource** | `expand_state(&self, s) -> StateExpansion` | immutable (`&self`) | duallity's specialized cache and callers that already own lifecycle policy |
| **LazyWfst** | `expand(&mut self, s) -> Result<ExpansionStatus, ExpansionError>`, `transitions_lazy(&mut self, s)` | mutable (`&mut self`) | direct callers, through `LazyStateCache` |

The immutable `StateSource` path is what makes composition cheap: `compose` holds a *shared* reference
and calls `compute_state` as the search visits product states, never needing `&mut`. The managed method
checks cancellation and delegates to the same `expand_state` kernel used by the specialized cache.
**Every** engine implements both source traits, but they divide by *when* their state set is fixed:
...
- **Lazily registering** — `LevenshteinStateSource`, `UniversalLevenshteinStateSource`,
  ...
  `expand_state`. It stays `&self` only because the registries live behind
...
- **Construction-fixed** — `WallBreakerWfst` and `RewriteWfst` need no interior mutability at all: they
  ...
  ($`0 \ldots C`$) — so their `expand_state` is a pure read.
