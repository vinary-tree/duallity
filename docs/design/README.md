...
This section documents each variant in depth: its exact 4.0.0-rc.6 API, its operational semantics, its
...
is the query length in Unicode scalars, $`k = \texttt{max\_distance}`$, $`M`$ is the state-encoding
...
|---------|----------------|-----------------------|--------|-------------------------------|---------|---------------|
| [**Levenshtein**](levenshtein-wfst.md) | `LevenshteinWfst<D>` | $`(d,\ \mathsf{N}(i,e))`$ + continuation cells; **arithmetic** $`a`$ | $`(n{+}1)(k{+}1)(1{+}c)`$ | fresh automaton **per query**: build $`\mathcal{O}(n)`$, expand $`\mathcal{O}(\delta(1{+}c))`$/state | none | rebuilds per query; eager reads need a prior `expand`; `k`: `usize` |
| [**Universal**](universal-wfst.md) | `UniversalLevenshteinWfst<V,D>`, `BoundUniversalWfst<V,D>` | $`(d,\ \Pi)`$; **registry** ids (both components) | $`(n{+}1)^2(2k{+}1)`$ | factory $`\mathcal{O}(1)`$; `with_query` $`\mathcal{O}(n(n{+}k))`$; $`U_k`$ needs no per-query build; $`\lvert\Sigma\rvert`$-independent | none | $`k \le 255`$ (`u8`); weight-$`\bar{1}`$ edges ⇒ pruning only at acceptance |
...
| [**Rewrite**](phonetic-rewrite-wfst.md) | `RewriteWfst`, `RewriteRule`, `CommonPhoneticRules` | state $`0`$ home/accepting + continuations; **no dictionary** | $`1 + \text{continuations}`$ | from a rule list; $`\texttt{num\_states()} = 1 + \text{continuations}`$ | none | rules apply **unconditionally** (no context/lookahead); expand context into explicit rules |
...
- **state ids** — a product state $`(d, a)`$ — a dictionary-node id $`d`$ and an
  ...
  $`\mathrm{StateId} = d \cdot M + a`$ with radix $`M = \texttt{max\_automaton\_states}`$
