...
$`q = \texttt{"helo"}`$ up to edit distance $`k = 2`$, walking the dictionary `dict`, exposed as
...
| Phase | Cost | Notes |
|-------|------|-------|
| **Construction** (`new` / `with_algorithm`) | $`\mathcal{O}(n)`$ + one dictionary-handle clone | collects $`q`$ into scalars, sizes the codec, registers the root; **no** dictionary traversal |
| **Per-state expansion** (`expand`) | $`O\bigl(\delta \cdot (1 + c)\bigr)`$ | one pass over the node's out-edges, $`\mathcal{O}(1)`$ arcs and one amortized-$`\mathcal{O}(1)`$ registry insert per edge, plus one delete arc |
...
| **Space** | $`\mathcal{O}(\lvert R\rvert)`$ cached states | bounded by the LRU cap when `CachePolicy::Lru` is set |
...
Take $`D = \{\texttt{"hello"}, \texttt{"help"}, \texttt{"world"}\}`$, query
$`q = \texttt{"helo"}`$
...
**Path to `"help"` (edit distance 1 — one substitution $`\texttt{o} \to \texttt{p}`$).** `h:h/0`, `e:e/0`, `l:l/0`, then
...
**Transposition: $`\texttt{"tset"} \to \texttt{"test"}`$ (Damerau distance 1).** With `Algorithm::Transposition`
over $`D = \{\texttt{"test"}\}`$, $`q = \texttt{"tset"}`$, $`k = 2`$:
...
The input tape reads $`\texttt{t,s,e,t} = q`$ and the output tape reads
$`\texttt{t,e,s,t} = \texttt{"test"}`$;
