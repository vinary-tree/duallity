...
Because a positive class enumerates its own small explicit ranges (up to
`MAX_EXACT_CLASS_RANGE_CHARS = 256` scalars each), `[a-c]` is exact even with an empty custom alphabet. Negated classes and `.`
...
grouping uses an $`\mathcal{O}(\lvert\Sigma\rvert)`$ indexed structure once the candidate set is large
