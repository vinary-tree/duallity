...
> ($`\texttt{ph} \to \texttt{f}`$, $`\texttt{ck} \to \texttt{k}`$, …) presented as a composable `Wfst<char, TropicalWeight>`.
...
Each **rule** maps an input substring to an output substring at a fixed tropical cost — $`\texttt{ph} \to \texttt{f}`$ at
$`0.1`$, $`\texttt{ck} \to \texttt{k}`$ at $`0.1`$, and so on — and every un-rewritten printable-ASCII character
...
A one-step rule ($`s_r = 1`$, e.g. $`\texttt{c} \to \texttt{s}`$) is a single self-loop $`0 \to 0`$ carrying the
...
representative of each key. A rule such as $`\texttt{ph} \to \texttt{f}`$ at $`0.1`$ therefore dominates the identity
...
Each preset is a `Vec<RewriteRule>` with `priority = 0` throughout; the $`\texttt{(input} \to \texttt{output, cost)}`$ rows
...
|----------------|------|-----|-------|------|
| $`\texttt{ph} \to \texttt{f}`$  | `0.1` | 2 | 1 | many-to-one |
| $`\texttt{gh} \to \texttt{f}`$  | `0.2` | 2 | 1 | *rough → ruff* |
| $`\texttt{ck} \to \texttt{k}`$  | `0.1` | 2 | 1 | many-to-one |
| $`\texttt{qu} \to \texttt{kw}`$ | `0.1` | 2 | 1 | two-to-two |
| $`\texttt{x} \to \texttt{ks}`$  | `0.1` | 2 | 1 | one-to-many |
| $`\texttt{c} \to \texttt{k}`$   | `0.2` | 1 | 0 | coarse hard-c |
| $`\texttt{c} \to \texttt{s}`$   | `0.2` | 1 | 0 | coarse soft-c |

**`CommonPhoneticRules::german()`** — $`\sum (s-1) = 3`$ ($`\texttt{sch} \to \texttt{sh}`$ alone contributes 2):

| input → output | cost | $`s`$ | $`s-1`$ | note |
|----------------|------|-----|-------|------|
| $`\texttt{sch} \to \texttt{sh}`$ | `0.1` | 3 | 2 | three-to-two |
| $`\texttt{ch} \to \texttt{x}`$   | `0.1` | 2 | 1 | voiceless fricative (IPA [x] / [ç]) |
| $`\texttt{ß} \to \texttt{ss}`$   | `0.1` | 2 | 1 | one-to-two |
| $`\texttt{ä} \to \texttt{ae}`$   | `0.1` | 2 | 1 | one-to-two |
| $`\texttt{ö} \to \texttt{oe}`$   | `0.1` | 2 | 1 | one-to-two |
| $`\texttt{ü} \to \texttt{ue}`$   | `0.1` | 2 | 1 | one-to-two |

**`CommonPhoneticRules::french()`** — $`\sum (s-1) = 5`$:

| input → output | cost | $`s`$ | $`s-1`$ | note |
|----------------|------|-----|-------|------|
| $`\texttt{eau} \to \texttt{o}`$ | `0.1` | 3 | 2 | three-to-one |
| $`\texttt{aux} \to \texttt{o}`$ | `0.1` | 3 | 2 | three-to-one |
| $`\texttt{ai} \to \texttt{e}`$  | `0.1` | 2 | 1 | two-to-one |
| $`\texttt{ph} \to \texttt{f}`$  | `0.1` | 2 | 1 | many-to-one |
| $`\texttt{qu} \to \texttt{k}`$  | `0.1` | 2 | 1 | two-to-one |

The German scalars `ß`, `ä`, `ö`, `ü` are single Unicode `char`s ($`\lvert\mathrm{in}\rvert = 1`$),
so $`\texttt{ß} \to \texttt{ss}`$ is one-to-many with $`s = 2`$.
...
$`\mathcal{O}(L)`$ time and space, with all buffers **preallocated** to their exact final sizes; the
priority order is one $`\mathcal{O}(R \log R)`$ sort. The automaton has $`\lvert Q \rvert = 1 + C`$
...
Pruning is $`\mathcal{O}(t)`$ linear scan for $`t < 16`$ edges and switches to an
$`\mathcal{O}(t)`$ `FxHashMap` pass at or above that threshold (`HASH_PRUNE_THRESHOLD`). Expanded states
...
|------|--------|--------|-------------|
| $`\texttt{ph} \to \texttt{f}`$ (many-to-one) | $`p : f / 0.1 \to c_1`$ | $`h : \varepsilon / \bar{1} \to 0`$ | shortens the **input** tape (trailing $`h : \varepsilon`$) |
| $`\texttt{f} \to \texttt{ph}`$ (one-to-many) | $`f : p / 0.1 \to c_1`$ | $`\varepsilon : h / \bar{1} \to 0`$ | lengthens the **output** tape (trailing $`\varepsilon : h`$) |
| $`\texttt{c} \to \texttt{s}`$ (one-to-one)   | $`c : s / 0.2 \to 0`$ | — | equal-length, single self-loop |

## 6. ⚠ Honest limitations

- **Rules are unconditional.** `RewriteRule` stores only `input`/`output`/`cost`/`priority`; there is
  no left/right context, lookahead, or word-boundary condition. The English $`\texttt{c} \to \texttt{k}`$ and $`\texttt{c} \to \texttt{s}`$ presets
...
The $`\texttt{ph} \to \texttt{f}`$ (many-to-one) and $`\texttt{f} \to \texttt{ph}`$ (one-to-many) chains, with the whole cost on step 0 and free
