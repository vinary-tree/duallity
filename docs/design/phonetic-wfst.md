...
[`RewriteWfst`](phonetic-rewrite-wfst.md) (fixed `ph→f`-style rules, feature-free). `PhoneticWfst` is
...
`build_from_pattern` is `regex::parse` → `nfa::compile` (a Thompson construction) wrapped into
...
| `bone`  | no  | `` $`1`$ `` (`b→f`) | `` $`1.5 \cdot 1 = 1.5`$ `` | `` $`0.1 \cdot 4 = 0.4`$ `` | `` $`1.9`$ `` |
...
`bone` is not in the pattern language; it is reached only through one edit (`b→f`, within
...
<img src="../diagrams/triple-product-frontier.svg" alt="A PhoneticWfst state (d, Φ) carries a deletion-closed frontier Φ of NFA × Levenshtein positions; each dictionary character c advances Φ to the successor frontier Φ′ = transition_frontier(Φ, c), and acceptance takes the minimum accepting edit distance over Φ (diagram pending central render)" width="820"/>
