...
$`\Pi_0 = \texttt{UniversalState::initial(k)}`$ at cursor $`j = 0`$ is registered as id
...
| Phase | Cost | Notes |
|-------|------|-------|
| **`BoundUniversalWfst::new`** | $`\mathcal{O}(1)`$ | moves the dictionary in, stores $`k`$; no traversal, no automaton build |
...
| **Per-state expansion** | $`\mathcal{O}(\delta)`$ | one characteristic-vector transition and one amortized-$`\mathcal{O}(1)`$ registry insert per dictionary edge, plus one deletion-continuation arc |
| **Space** | $`\mathcal{O}(\lvert R\rvert)`$ cached states | bounded by the LRU cap under `CachePolicy::Lru` |
...
| `"help"` | substitute $`\texttt{o} \to \texttt{p}`$ | all $`\bar{1} = 0`$ | $`1`$ |
...
- **`with_query` is not zero-cost.** The query-agnostic $`U_k`$ needs no rebuild, but each
  ...
  windows ($`\mathcal{O}(n(n+k))`$). The amortized win over `LevenshteinWfst` is the eliminated per-query
