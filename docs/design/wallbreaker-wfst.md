...
Complexity: $`\mathcal{O}(R)`$ arcs from $`q_0`$ (one per non-empty term), $`\mathcal{O}(1)`$ from any
interior state.
...
lookup and gives $`\mathcal{O}(1)`$ UTF-8-correct character access at any $`\langle r, p\rangle`$.
...
$`\mathcal{O}(k)`$ pieces, the exact-substring seed per piece (SCDAWG, linear in piece length and
independent of $`k`$), the $`k`$-bounded bidirectional extension, and the exact
re-verification (theory/06) — plus the wrapper's $`\mathcal{O}(R_{\text{raw}})`$ hash-dedup normalization
and $`\mathcal{O}(C)`$ arena/state-index build, where $`C = \sum_r m_r`$ is the total matched-term
length. **Space** is $`\mathcal{O}(C)`$ for the arena and $`\lvert Q\rvert = 1 + C`$ for the state
...
(substitute $`\texttt{o} \to \texttt{p}`$); `"world"` is out of bound. Suppose normalization yields $`w_0 = \texttt{"hello"}`$
