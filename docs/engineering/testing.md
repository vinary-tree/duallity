...
- `src/universal_state_source.rs::test_universal_state_source_can_spell_full_label_pairs` — the
  ...
  $`(\texttt{query}, \texttt{term}) \in \{(\texttt{"at"},\texttt{"cat"}), (\texttt{"cat"},\texttt{"at"}), (\texttt{"axb"},\texttt{"ab"})\}`$
  still expose a
...
|------|------|------------------|-------------------------|
| `rewrite_wfst_one_to_many_output_chain_emits_continuation` | $`\texttt{f} \to \texttt{ph}`$ | `(Some('f'), Some('p'), 0.1)` | `(None, Some('h'), 0)` — output $`\varepsilon`$-emits `h`, weight `one()` $`= \bar{1} = 0`$, back to state `0` |
| `rewrite_wfst_many_to_one_input_chain_consumes_continuation` | $`\texttt{ph} \to \texttt{f}`$ | `(Some('p'), Some('f'), 0.1)` | `(Some('h'), None, …)` — input $`\varepsilon`$-consumes `h`, back to state `0` |
...
1. **Place the file by feature.** A default-feature variant belongs in `tests/<variant>.rs` with no
   gate. For a variant that needs `phonetic-rules`, add `#![cfg(feature = "phonetic-rules")]` at the top of the
   file.
...
6. **Exercise the cache dimension:** `clear_cache()` empties `computed_states()`, and a non-default
   `CachePolicy` behaves as specified: `NoCache` is scratch-only, while `Lru { max_states }` evicts.
