...
the state source's `compute_state(s)`, and stores a compact `CachedState`; the second touch is served
...
- A query that visits `` $`N`$ `` states costs `` $`O(N)`$ `` expansions — **independent of dictionary
...
`` $`O(nk)`$ ``-narrow, small `` $`k`$ `` is cheap and the cost grows only where the dictionary and the
band overlap.
...
|---------|--------------------------|---------------------|-----------|
| [`LevenshteinWfst`](../design/levenshtein-wfst.md) | `` $`O(n)`$ `` + one dict-handle clone; **no** dictionary walk | `` $`O\bigl(\delta(1{+}c)\bigr)`$ `` | **lazy** |
| [`BoundUniversalWfst`](../design/universal-wfst.md) (factory) | `` $`O(1)`$ `` — captures `` $`(D, k)`$ `` | — | — |
| └ `with_query` → `UniversalLevenshteinWfst` | `` $`O\bigl(n(n{+}k)\bigr)`$ ``; **no automaton rebuild**, `` $`\lvert\Sigma\rvert`$ ``-independent | `` $`O(\delta)`$ `` | **lazy** |
...
| [`WallBreakerWfst`](../design/wallbreaker-wfst.md) | **runs the whole query**: `` $`O(k)`$ ``-piece split, SCDAWG seed (linear in piece length, `` $`k`$ ``-independent), `` $`k`$ ``-bounded extend, verify, + `` $`O(R)`$ `` dedup + `` $`O(C)`$ `` arena | `` $`q_0 \to {\le}R`$ `` arcs; interior `` $`\to {\le}1`$ `` arc | **eager** |
| [`RewriteWfst`](../design/phonetic-rewrite-wfst.md) | `` $`O(L)`$ `` tokenize (preallocated) + `` $`O(R \log R)`$ `` priority sort | home: `` $`R{+}95`$ `` (or `` $`R`$ ``) edges, then prune; continuation: `` $`1`$ `` | **lazy** |
...
query-agnostic `` $`U_k`$ `` once** and mints per-query wrappers at `` $`O(n(n{+}k))`$ `` with no
...
- traversal of the built forest is cheap and `` $`k`$ ``-independent: expanding the super-start yields
  `` $`\le R`$ `` arcs and any interior state `` $`\le 1`$ ``, so the whole view is `` $`O(C)`$ ``;
...
relevant-subword precompute and registry seeding (`` $`O(n(n{+}k))`$ ``), and it is
