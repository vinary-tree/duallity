...
- **Dictionary capture is $`\mathcal{O}(1)`$.** No terms are serialized or copied; the snapshot is a
  structural-sharing handle (the persistent-revision model of the capture-once rule).
- **Resource handoff is $`\mathcal{O}(1)`$.** `duallity_wfst_resource` returns a two-word `VtResource`
  containing `context` and `vtable`, with one added retain — no graph is materialized or marshaled.
...
`Arc<[char]>` for per-scalar edit distance. That is $`\mathcal{O}(n)`$ in the query length $`n`$, not
...
|-----------|----------------|
| crate `duallity` | `4.0.0-rc.6` (edition 2021, **MSRV 1.95**) |
| `vinary-tree-interop` | `4.0.0-rc.6` (ABI version `1`) |
| npm `@vinary-tree/duallity` | `4.0.0-rc.6` |
| npm `@vinary-tree/vinary-tree-interop` (dependency) | `4.0.0-rc.6` |
| npm `@vinary-tree/javascript-runtime` (runtime) | `4.0.0-rc.6` |
| Node engine | `>= 22.14` |
| Julia `Duallity.jl` / `VinaryTreeInterop.jl` | `4.0.0-rc.6` / major `4` |
| Julia | `>= 1.10` |
| Raku `Duallity` | `4.0.0-rc.6` (language `6.d`) |

The crate and npm facade resolve exact `4.0.0-rc.6` family dependencies. Use
