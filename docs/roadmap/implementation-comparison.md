...
**Note**: liblevenshtein remains an **external library**. MORK's FuzzySource is an adapter that calls liblevenshtein—it does not contain fuzzy matching code. See MORK FuzzySource Adapter *(external design — not part of this repo)* for architecture details.
