...
dictionary node, so it runs in `` $`O(\deg(d))`$ `` time (where `` $`\deg(d)`$ `` is the number of
...
`TropicalWeight::new(f64::from(operation_cost))` with `operation_cost ∈ {0,1}` for match/substitute,
...
**Proof outline.** We first dispatch a bookkeeping subtlety (§5.1), then import the path`` $`\leftrightarrow`$ ``alignment
...
*Merge* (`` $`\langle 1`$ `` dict char, `` $`2`$ `` query chars`` $`\rangle`$ ``). If `` $`i+1 < n`$ ``
...
*Split* (`` $`\langle 2`$ `` dict chars, `` $`1`$ `` query char`` $`\rangle`$ ``). If `` $`i < n`$ ``
...
### 8.1 A full accepting path: `"helo"` `` $`\to`$ `` `"hello"`
...
4. **Mohri, M., Pereira, F., & Riley, M.** (2002). *Weighted Finite-State Transducers in Speech
   ...
   semantics `` $`T(x,y) = \bigoplus_{\pi} w(\pi) \otimes \rho(\pi)`$ ``, so shortest path `` $`=`$ ``
   best answer.
