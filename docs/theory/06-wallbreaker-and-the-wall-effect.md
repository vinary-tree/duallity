...
$`\mathcal{O}(\lvert P_j \rvert)`$ per piece and independent of $`k`$):
...
$`\delta_i`$ ($`\texttt{result\_final\_weights}[i] = \texttt{distance\_to\_f64}(\delta_i)`$). By the
...
search is $`\mathcal{O}(\lvert P_j \rvert)`$ per piece and independent of $`k`$, followed by bounded
extension and verification), materialising $`R`$. *Per query, once constructed*, the WFST is an
**$`\mathcal{O}(1)`$-expansion view**: every non-root state lazily expands to **at most one** outgoing
