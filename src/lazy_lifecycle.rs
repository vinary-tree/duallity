...
///
/// lling-llang's [`StateSource`](lling_llang::prelude::StateSource) receives an
/// [`ExpansionRequest`] so its lazy wrapper can enforce snapshots, attempts,
/// and cooperative cancellation. This companion trait exposes the underlying
/// deterministic state computation for tests, specialized caches, and
/// downstream integrations that already manage those lifecycle concerns.
